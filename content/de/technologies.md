---
title: Technologien
description: Wie sich Greenmice Geosolutions und Greenmice Technologies verhalten
  – und wann man die einzelnen Unternehmen engagieren sollte.
translationKey: technologies-bridge
menu:
  main:
    identifier: technologies-bridge
    weight: 60
  footer:
    identifier: technologies-bridge
    weight: 60
---

Greenmice operates two engineering companies with clear boundaries:

### Greenmice Geosolutions (this site)

Focused on **GIS engineering**, **spatial data platforms**, and **aeronautical systems**—including long-running collaboration with **IDS AirNav** in the **ENAV** ecosystem.

### Greenmice Technologies

Focused on **AI systems**, **enterprise software engineering**, **legacy modernization**, **mobile applications**, and **CTO as a Service**.

**Website:** [greenmice.it](https://greenmice.it/)

### When projects span both

Many roadmaps need spatial systems *and* core enterprise platforms. In those cases we coordinate delivery across both companies: shared engineering culture, separate ownership lines, and explicit interfaces—so accountability stays clear.
